...
* [`src/clj/zanychords/`](src/clj/zanychords/): CSS compilation source files (Clojure,
[Garden](https://github.com/noprompt/garden))
* [`src/cljs/zanychords/`](src/cljs/zanychords/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/zanychords/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `zanychords` project root directory
...
in the [`src/clj/zanychords/`](src/clj/zanychords/) directory. CSS files are compiled
...
The `debug?` variable in [`config.cljs`](src/cljs/zanychords/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns zanychords.example
  (:require [zanychords.config :as config])
